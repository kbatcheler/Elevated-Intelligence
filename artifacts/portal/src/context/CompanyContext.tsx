...
import {
  ...
  type VerifiedClaim, type ModelledClaim, type HeroPanel, type PeerBenchmark, type SupplementBlocks,
} from "../data/layers";
...
// Tenant-scope brief overrides (Phase 2 sub-stage 9). Drives MorningBrief +
// BoardPack copy. Nullable for tenants seeded before this stage existed;
// fillable via POST /api/tenants/:id/panels/backfill.
export interface ServerBriefOverrides {
  executiveRead: string;
  pullQuote: string;
  combinedRecovery: string;
  recoveryConfidence: string;
  topFindings: Array<{ layerKey: string; finding: string; impact: string; lever?: string }>;
  rootCauses: Array<{ title: string; impact: string; body: string }>;
  recoveryLevers: Array<{ title: string; horizon: string; recovery: string; owner: string; body: string }>;
}

export interface TenantDetail {
  tenant: TenantSummary;
  profile: ServerProfile;
  briefOverrides?: ServerBriefOverrides | null;
  layers: Array<{
    layerKey: string;
    ...
    // Optional peer benchmark (Phase 2 sub-stage 7). Null for tenants
    // seeded before this column existed; fillable via POST
    // /api/tenants/:id/panels/backfill.
    peerBenchmark?: PeerBenchmark | null;
    // Optional supplement blocks (Phase 2 sub-stage 8).
...
function projectProfile(
  ...
  // Brief overrides are tenant-scope copy generated by Phase 2 sub-stage 9.
  // When present, they win for the lede / pull-quote / root causes / recovery
  // levers / combined recovery / per-layer findings. When absent (older
  // tenants or backfill not yet run), the briefs fall back to neutral
  // generic copy at the component level.
  const topFindingsMap: Record<string, { finding: string; impact: string; lever?: string }> = {};
  if (briefOverrides) {
    for (const f of briefOverrides.topFindings) {
      const entry: { finding: string; impact: string; lever?: string } = {
        finding: f.finding,
        impact: f.impact,
      };
      if (f.lever) entry.lever = f.lever;
      topFindingsMap[f.layerKey] = entry;
    }
  }
  return {
    id: t.id,
    ...
    executiveRead: briefOverrides?.executiveRead ?? p.executiveRead,
    pullQuote: briefOverrides?.pullQuote ?? p.pullQuote,
    topFindings: briefOverrides ? topFindingsMap : undefined,
    rootCauses: briefOverrides?.rootCauses,
    recoveryLevers: briefOverrides?.recoveryLevers,
    combinedRecovery: briefOverrides?.combinedRecovery,
    recoveryConfidence: briefOverrides?.recoveryConfidence,
...
function projectNarrative(detail: TenantDetail | null): NarrativeBundle {
  ...
  const layers = detail.layers
    .map(l => projectLayer(
      ...
      l.heroPanel ?? null,
      l.peerBenchmark ?? null,
      l.supplementBlocks ?? null,
    ))
...
export function CompanyProvider({ children }: { children: ReactNode }) {
  ...
  const profile = useMemo<CompanyProfile>(() => {
    ...
    return projectProfile(activeTenant, detail.profile, detail.briefOverrides ?? null);
  }, [detail, activeTenant]);
