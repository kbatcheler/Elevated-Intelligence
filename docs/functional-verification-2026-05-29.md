...
- **Per-gap confidence lift was always "+0pp" (FIXED — data efficacy).** Every layer's gap list, the ConfidenceLadder visual, and the EngagementPipeline weighting showed "+0pp confidence" for closing any gap. Root cause: the Score stage computes a grounded per-gap `confidence_lift_pp`, but `phase2.ts` dropped that field when merging the gaps into the stored layer content, and the portal read a non-existent key (`confidence_gap`). Fix: the field is now allowed in the content gaps schema, preserved (with score-stage gaps taking precedence so they are not truncated), and read correctly in the portal. The three already-ready tenants (Stripe, Patagonia, Twilio) were re-seeded on 2026-05-29 against the corrected code so they now carry real lift values (see "Stale boot splash" and "Moving progress counter" below).
- **Stale "in flight" rows on a failed or orphaned run (FIXED).** When `failRun` or the boot reconciler set a run to `failed`, the per-stage states were left untouched, so the boot splash kept showing "Layers IN FLIGHT 0/14" next to the failure controls. Fix: a pure `sealRunningStages` helper (`runner-helpers.ts`) now flips every non-terminal stage and its in-flight layer/sub-stage rows to `failed` while leaving `pending` rows as-is; `failRun` seals inside the same serialized write queue, and the reconciler seals each orphaned run. A run-status guard was also added to `updateStage` and `syncLayerEntry` so a straggling layer worker that finishes after a run is sealed can no longer flip stages back to a non-terminal state. The three pre-existing `failed` runs for "Little Spoon" were also backfilled so their stages no longer read "in flight".
- **Progress counter sat at 0/14 during the seed (FIXED).** The Layers counter only advanced when a whole layer's sub-stage chain finished, so it read `0/14` for most of the ~40-minute run. Fix: progress is now measured in sub-stages (14 layers x 8 sub-stages = 112 total). `syncLayerEntry` recomputes `{current: completed sub-stages, total: 112}` on every flush, and the splash shows "Layers · 14 intelligence frames · {pct}%". Verified live on the 2026-05-29 re-seeds: progress reports `total: 112` with `current` climbing instead of frozen.

### Major (visible to user, affects core value)
- **Path 2, Error feedback. A failed or orphaned run left stale "in flight" stage rows. (NOW FIXED — see "Stale 'in flight' rows" under Fixed in this pass.)** The boot splash already showed a failure affordance (Retry / relabelled Dismiss), but `failRun` / the reconciler set the run to `failed` without rewriting the per-stage states, so the splash kept displaying "Layers IN FLIGHT 0/14" alongside the failure controls. This is now sealed at failure time, on reconcile, and guarded against late worker writes; the three historical "Little Spoon" failed runs were backfilled.

### Minor (edge case, workaround exists)
- **Progress counter looked stuck during seeding. (NOW FIXED — see "Progress counter sat at 0/14" under Fixed in this pass.)** Progress is now measured in sub-stages (112 total) and recomputed on every flush, so the splash advances continuously instead of sitting at `0/14`.
...
- Code was changed in this pass (four fixes; see "Fixed in this pass"). No tenants were deleted. The three ready tenants were re-seeded and three historical failed runs had their stale stages backfilled; the database was otherwise read only.

## Verdict

**Functional; all four functional findings now fixed.** Seeding works (it takes about 40 minutes under current rate limits, which is expected, not a failure). The blank "No tenant" dashboard, the always-"+0pp" gap confidence lift, the stale boot-splash state on a failed run, and the stuck progress counter are all fixed. The three ready tenants (Stripe, Patagonia, Twilio) were re-seeded on 2026-05-29 against the corrected code so they pick up real per-gap confidence lift; those re-seeds run for ~40 minutes each.
