...
// Phase 3.4 dead-link reporting. Every time a reader clicks "Report broken
// link" inside a VerifiedPill tooltip we insert one row here. Intentionally
// append-only and lightweight: no de-duplication, no rate-limit at the
// schema layer (rate-limit lives in the route). Tenant cascade-deletes so
// removing a tenant cleans up the reports too. tenantId is nullable so the
// surface still works if the click ever fires before the active-tenant
// hydration finishes.
